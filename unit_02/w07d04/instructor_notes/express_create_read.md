...
So in your `/unit_02/w05d03/student_labs` folder, you should see a starter application named `starter-code-hbs` with a bunch of work done already to get us going.

<br>

&#x1F535; **YOU DO: Starter Code Set-up**

1. `cd` into `starter-code-hbs` and `atom .`. 
2. Run the command `npm install` which will read the `package.json` file and install all of the dependencies specified. 
...
1. Now, we can access our `seededTodos` via our `data.js` file like so:
...
models and more routes to our application.  Keeping everything in `server.js` will quickly become unwieldy.
...
<details>
    <summary>_Nested Resources_</summary>
...
2. Inside of `/todos/new.hbs` build a basic html form:
...
```
    ...
    `POST`. What's the route we need to `POST` to?

    `/todos`


    What did we learn about html forms in unit 1? All forms must have a `method` and an `action`.
...
The `name` attribute contains our key. What the user types into the text field or slects as a radio button is our `value`. This would get parsed server side as:
